...
#ifdef JAKUBELIB_MULTITHREADED_BUILD
#include <thread>
#include <mutex>
...
#ifdef _MSC_VER
// Needed for Visual Studio to disable runtime checks for memcpy
#pragma runtime_checks("s", off)
#endif

// This allows others to supply their own logger / error printer without
// requiring Jakube to import their headers. See RcppJakube for a use case.
...
#define JAKUBELIB_USE_AVX512
#elif !defined(NO_MANUAL_VECTORIZATION) && defined(__AVX__) && defined(__SSE__) && defined(__SSE2__) && defined(__SSE3__)
#define JAKUBELIB_USE_AVX
#else
#endif

#if defined(JAKUBELIB_USE_AVX) || defined(JAKUBELIB_USE_AVX512)
#if defined(_MSC_VER)
#include <intrin.h>
...
#define JAKUBELIB_FTRUNCATE_SIZE(x) static_cast<int64_t>(x)
#else
...
template<typename S, typename T, typename Distance, typename Random, class ThreadedBuildPolicy>
  class JakubeIndex : public JakubeIndexInterface<S, T,
#if __cplusplus >= 201103L
...
public:
   ...
   JakubeIndex(int f) : _f(f), _seed(Random::default_seed) {
  ...
  bool on_disk_build(const char* file, char** error=NULL) {
    ...
    _nodes_size = 1;
    if (ftruncate(_fd, JAKUBELIB_FTRUNCATE_SIZE(_s) * JAKUBELIB_FTRUNCATE_SIZE(_nodes_size)) == -1) {
  ...
  bool build(int q, int n_threads=-1, char** error=NULL) {
    ...
    if (_verbose) jakubelib_showUpdate("has %d nodes\n", _n_nodes);
  ...
  void unload() {
    ...
    reinitialize();
    if (_verbose) jakubelib_showUpdate("unloaded\n");
  }
  ...
  bool load(const char* filename, bool prefault=false, char** error=NULL) {
    ...
    if (prefault) {
      ...
      flags |= MAP_POPULATE;
#else
      jakubelib_showUpdate("prefault is set to true, but MAP_POPULATE is not defined on this platform");
#endif
    }
    ...
    _built = true;
    _n_items = m;
    if (_verbose) jakubelib_showUpdate("found %zu roots with degree %d\n", _roots.size(), m);
    return true;
  }
  ...
  void thread_build(int q, int thread_idx, ThreadedBuildPolicy& threaded_build_policy) {
    ...
    while (1) {
      ...
      if (_verbose) jakubelib_showUpdate("pass %zd...\n", thread_roots.size());
...
protected:
  void _reallocate_nodes(S n) {
    ...
    if (_on_disk) {
      if (!remap_memory_and_truncate(&_nodes, _fd,
          ...
          _verbose)
          jakubelib_showUpdate("File truncation error\n");
    } else {
    ...
    if (_verbose) jakubelib_showUpdate("Reallocating to %d nodes: old_address=%p, new_address=%p\n", new_nodes_size, old, _nodes);
  }
  ...
  S _make_tree(const vector<S>& indices, bool is_root, Random& _random, ThreadedBuildPolicy& threaded_build_policy) {
    ...
    for (int attempt = 0; attempt < 3; attempt++) {
      ...
      for (size_t i = 0; i < indices.size(); i++) {
        ...
        if (n) {
          ...
          jakubelib_showUpdate("No node for index %d?\n", j);
        }
      }
    ...
    while (_split_imbalance(children_indices[0], children_indices[1]) > 0.99) {
      if (_verbose)
        jakubelib_showUpdate("\tNo hyperplane found (left has %zu children, right has %zu children)\n",
...
public:
  ...
  static void build(JakubeIndex<S, T, D, Random, JakubeIndexSingleThreadedBuildPolicy>* jakube, int q, int n_threads) {
    JakubeIndexSingleThreadedBuildPolicy threaded_build_policy;
    jakube->thread_build(q, 0, threaded_build_policy);
  }
...
#ifdef JAKUBELIB_MULTITHREADED_BUILD
class JakubeIndexMultiThreadedBuildPolicy {
private:
...
public:
  template<typename S, typename T, typename D, typename Random>
  static void build(JakubeIndex<S, T, D, Random, JakubeIndexMultiThreadedBuildPolicy>* jakube, int q, int n_threads) {
    JakubeIndexMultiThreadedBuildPolicy threaded_build_policy;
    if (n_threads == -1) {
    ...
    for (int thread_idx = 0; thread_idx < n_threads; thread_idx++) {
      ...
      threads[thread_idx] = std::thread(
        &JakubeIndex<S, T, D, Random, JakubeIndexMultiThreadedBuildPolicy>::thread_build,
        jakube,
        trees_per_thread,
        thread_idx,
